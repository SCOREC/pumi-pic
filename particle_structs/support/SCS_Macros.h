...
#define PS_PARALLEL_FOR_ELEMENTS(SCS, thread, element_id, ALGORITHM)    \
  ...
  kkLidView offsets = SCS->get_offsets();                               \
  kkLidView slice_to_chunk = SCS->get_slice_to_chunk();                 \
  kkLidView row_to_element = SCS->get_row_to_element();                 \
  kkLidView particle_mask = SCS->get_particle_mask();                   \
  const int league_size = SCS->get_num_slices();                        \
