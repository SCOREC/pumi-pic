...
typedef particle_structs::CabM<Type, exe_space> CabM;
...
bool destroyConstructor(int ne_in, int np_in, int distribution) {
  ...
  CabM::kkLidView ptcls_per_elem_v("ptcls_per_elem_v", ne_in);
  CabM::kkGidView element_gids_v("element_gids_v", ne_in);
  ...
  CabM* cabm = new CabM(po, ne_in, np_in, ptcls_per_elem_v, element_gids_v);
  delete cabm;
  
  cudaMemGetInfo(&free, &total);
  const long used_after=total-free;
  if (used_before != used_after) {
    fprintf(stderr, "[ERROR] CabM has allocated too much memory\n");
    fails += 1;
  }
  return fails;
}
...
bool destroyRebuildSwap(int ne_in, int np_in, int distribution) {
  ...
  const long used_after=total-free;
  if (used_before != used_after) {
    fprintf(stderr, "[ERROR] CabM::rebuild has allocated too much memory\n");
    fails += 1;
  }
...
bool destroyMigrateSwap(int ne_in, int np_in, int distribution) {
  ...
  // check rebuild doesn't allocate extra memory
  CabM* cabm = new CabM(po, ne_in, np_in, ptcls_per_elem_v, element_gids_v);
  CabM::kkLidView new_element("new_element", cabm->capacity()); // just move particles to first element
  CabM::kkLidView new_process("new_process", cabm->capacity()); // just keep particles where they are
  int* comm_rank_h = new int[1];
  comm_rank_h[0] = comm_rank;
  CabM::kkLidView comm_rank_d("comm_rank_d", 1);
  particle_structs::hostToDevice(comm_rank_d, comm_rank_h);
  Kokkos::parallel_for("assign_same_process", cabm->capacity(),
