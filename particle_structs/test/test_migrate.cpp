  ...
  auto checkPostMigrate = PS_LAMBDA(const lid_t& e, const lid_t& p, const bool& mask) {
    if (mask) {
      ...
      if (e == num_elems - 1 && rank == local_rank) {
        Kokkos::printf("[ERROR] Failed to send particle %d on rank %d\n",
               pid, local_rank);
        failures(0) = 1;
      }
      if (e != num_elems - 1 && rank != local_rank) {
        Kokkos::printf("[ERROR] Incorrectly received particle %d from rank %d to rank %d in element %d\n", pid, rank, local_rank, e);
        failures(0) = 1;
      }
    }
  };
  ...
  auto checkPostBackMigrate = PS_LAMBDA(const lid_t& e, const lid_t& p, const bool& mask) {
    if (mask) {
      if (rnks(p) != local_rank) {
        Kokkos::printf("[ERROR] Test %s: Particle %d from rank %d was not sent back on rank %d\n",
  ...
  auto checkValues = PS_LAMBDA(const lid_t& elm_id, const lid_t& ptcl_id, const bool& mask) {
    if (mask) {
      ...
      if (Kokkos::fabs(rank*5 - val) > .0005) {
        Kokkos::printf("[ERROR] %d Value fails on ptcl %d (%d %.2f) on %s", comm_rank_local, ptcl_id, rank*5, val, name);
        failures(0) = 1;
      }
    }
  };
...
int migrateToEmptyAndRefill(const char* name, PS* structure) {
  ...
  if (comm_rank % 2 == 0 && (comm_rank != 0 || comm_size % 2 == 0)) {
    ...
    auto checkPtcls = PS_LAMBDA(const lid_t& e, const lid_t& p, const bool& mask) {
      if (mask) {
        failures(0) = 1;
        Kokkos::printf("[ERROR] Particle %d remains on rank %d\n", p, local_rank);
      }
    };
    ...
    auto checkPtcls = PS_LAMBDA(const lid_t& e, const lid_t& p, const bool& mask) {
      if (mask) {
        if (new_rnks(p) != local_rank && new_rnks(p) !=  prev_rank) {
          failures(0) = 1;
          Kokkos::printf("[ERROR] Particle %d is not from ranks %d or %d\n", p, local_rank, prev_rank);
        }
      }
    };
    ps::parallel_for(structure, checkPtcls, "checkPtcls");
