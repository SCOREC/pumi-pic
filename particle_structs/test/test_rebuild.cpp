...
int rebuildNoChanges(const char* name, PS* structure) {
  ...
  auto checkSameElement = PS_LAMBDA(const lid_t& e, const lid_t& p, const bool& mask) {
    if (mask) {
      ...
      if (dest_elem != e) {
        Kokkos::printf("[ERROR] Particle %d was moved to incorrect element %d on %s "
  ...
  auto checkElementSums = KOKKOS_LAMBDA(const int i) {
    ...
    if (old_sum != new_sum) {
      Kokkos::printf("[ERROR] Sum of particle ids on element %d do not match on %s. Old: %d New: %d\n",
...
int rebuildPtclsDestroyed(const char* name, PS* structure) {
  ...
  auto checkElement = PS_LAMBDA(const lid_t& e, const lid_t& p, const bool& mask) {
    if (mask) {
      ...
      const lid_t dest_elem = new_element(id);
      if (id%7 == 0) {
        Kokkos::printf("[ERROR] Particle %d (%d, %d) was not removed during rebuild on %s\n", id, e, p, name);
        failed(0) = 1;
      }
      else if (dest_elem != e) {
        Kokkos::printf("[ERROR] Particle %d was moved to incorrect element %d on %s "
    ...
    if (mask) {
      ...
      if (id < cap) { //Check old particles
        ...
        if (id % 7 == 0) { //Check removed particles
          Kokkos::printf("[ERROR] Particle %d in element %d was not removed during rebuild on %s\n", id, e, name);
          failed(0) = 1;
        }
        else if (dest_elem != e) {
          Kokkos::printf("[ERROR] Particle %d was moved to incorrect element %d on %s "
        ...
        const lid_t dest_elem = new_particle_elements(i);
        if (e != dest_elem) {
          Kokkos::printf("[ERROR] New particle %d was added to incorrect element %d on %s "
                 "(should be in element %d)\n", id, e, name, dest_elem);
          failed(0) = 1;
        }
      }
