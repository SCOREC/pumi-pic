...
%% Data Reading
fileID_rebuild = fopen( strcat('data/largeE_smallP_rebuild',int2str(bytes),'.dat'));
if not(migrate)
    fileID_push = fopen( strcat('data/largeE_smallP_push',int2str(bytes),'.dat'));
else
    fileID_push = fopen( strcat('data/largeE_smallP_migrate',int2str(bytes),'.dat'));
end
