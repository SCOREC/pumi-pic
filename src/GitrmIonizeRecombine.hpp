...
inline void ionization(const GitrmIonizeRecombine& gir, o::Real dt) {
  ...
  const auto& iRates = gitf.ionizationRates; 
  const auto coords = mesh.coords();
  const auto mesh2verts = mesh.ask_elem_verts();
  const auto tionVtx = o::Reals( mesh.get_array<o::Real>(o::VERT, "TionVtx"));
  const auto densVtx = o::Reals( mesh.get_array<o::Real>(o::VERT, "densityVtx"));
  auto pos_scs = scs->get<PTCL_POS>();
  auto xface_scs = scs->get<XPOINT_FACE>();
  auto charge_scs = scs->get<PTCL_CHARGE>();
  auto first_ionizeZ_scs = scs->get<FIRST_IONIZEZ>();
  auto prev_ionize_scs = scs->get<PREV_IONIZE>();
  auto first_ionizeT_scs = scs->get<FIRST_IONIZET>();
  auto lambda = SCS_LAMBDA(const int &elem, const int &pid, const int &mask) {
    if(mask >0) {
      o::LO debug = 1;
      auto pos = p::makeVector3(pid, pos_scs);
      auto charge = p::makeVector3(pid, charge_scs);
  	  auto bcc = o::zero_vector<4>();
      p::findBCCoordsInTet(coords, mesh2verts, pos, elem, bcc);
      // from tags
      o::Real tlocal = p::interpolateTet(tionVtx, bcc, elem, 3, i);
	    o::Real nlocal = p::interpolateTet(densVtx, bcc, elem, 3, i);
      if(charge > 74-1) //W=74, charge index=73
        charge = 0;
	    // from data array
			o::Real rate = interpolateRateCoeff(iRates, tlocal, nlocal, 
        gridT0, gridD0, dTem, dDens, nTRates, nDRates, charge);
			o::Real rateIon = 1/(rate*nlocal);
			if(p::almost_equal(tlocal,0) || p::almost_equal(nlocal, 0) 
				rateIon=1.0e12;

    	o::Real P = exp(-dt/rateIon);
    	o::Real P1 = 1.0-P;

      auto xfid = xface_scs(pid);
...
OMEGA_H_DEVICE o::Real interpolateRateCoeff(const o::Reals &data, 
  ...
  auto gridTnext = gridT0 + (indT+1)*dT;
  o::Real aT = pow(10.0, gridTnext) - tem;
  auto gridT = gridT0 + indT*dT;
  o::Real bT = tem - pow(10.0, gridT);
  o::Real abT = aT+bT;
  auto gridDnext = gridD0 + (indN+1)*dD;
  o::Real aN = pow(10.0, gridDnext) - dens;
  auto gridD = gridD0 + indN*dD;
